/**
 * GET /api/user/properties — Listar propiedades del usuario autenticado
 * POST /api/user/properties — Crear propiedad
 */
...
/** GET — Listar propiedades del usuario autenticado */
export async function GET(request: NextRequest) {
  ...
  let query = supabase
    .from("properties")
    .select(`*, property_images(id, url, alt_text, sort_order)`, { count: "exact" })
    .eq("user_id", userId)
  ...
  const properties = (data || []).map((p: any) => ({
    ...p,
    property_images: (p.property_images || []).sort((a: any, b: any) => a.sort_order - b.sort_order),
  }));
...
export async function POST(request: NextRequest) {
  ...
  const { data: property, error: propError } = await supabase
    .from("properties")
    .insert({
      user_id: userId, title: title.trim(), slug: finalSlug, description: description.trim(),
      deal_type, category, city, zone, currency, price: parseFloat(price.toString()),
      beds: beds ? parseInt(beds) : null, baths: parseInt(baths), area_m2: parseFloat(area_m2.toString()),
      lat: lat ? parseFloat(lat) : null, lng: lng ? parseFloat(lng) : null,
      amenities: amenitiesRaw, matricula_ref: matricula_ref?.trim() || null, status: "aprobada",
    })
    .select().single();
  ...
  return NextResponse.json({ property, message: "Propiedad creada exitosamente. Pendiente de revisión." }, { status: 201 });
}